...
async function loadStatusReports(){
  ...
  try{
    ...
    if(reports.length===0){ container.innerHTML='<p>ยังไม่มีรายการแจ้งซ่อม</p>'; return; }
  ...
  }catch(err){ console.error(err); container.innerHTML='<p>เกิดข้อผิดพลาดในการโหลดข้อมูล</p>'; }
}
...
// --- Delete ---
function showStatusDeletePopup(id){ statusDeleteTargetId=id; 
  const popup=document.getElementById('deleteModal'); popup.style.display='flex'; popup.classList.add('active'); 
}
...
document.getElementById('confirmDelete').onclick=async ()=>{
  ...
  try{
    ...
    const popup=document.getElementById('deleteModal'); popup.style.display='none'; popup.classList.remove('active');
    loadStatusReports();
  }catch(err){ console.error(err); alert('ไม่สามารถลบข้อมูลได้'); }
};
document.getElementById('cancelDelete').onclick=()=>{ statusDeleteTargetId=null; const popup=document.getElementById('deleteModal'); popup.style.display='none'; popup.classList.remove('active'); };
